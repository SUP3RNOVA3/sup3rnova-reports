...
function thumbnailElement(item,className='',eager=false){return item.thumbnailUrl?`<img class="media-thumbnail ${className}" src="${escapeHtml(item.thumbnailUrl)}" loading="${eager?'eager':'lazy'}" ${eager?'fetchpriority="high"':''} decoding="async" alt="${escapeHtml(item.type)} by ${escapeHtml(item.user)}">`:`<div class="media-placeholder ${className}"><span>${item.assetKind==='video'?'VIDEO':'MEDIA'}</span></div>`}
...
function mediaElement(item,options={}){
  const preload=options.preload||'metadata';const muted=options.muted===false?'':'muted';
  if(item.assetKind==='video'&&item.assetUrl)return `<video src="${escapeHtml(item.assetUrl)}" poster="${escapeHtml(item.thumbnailUrl)}" ${muted} playsinline preload="${preload}" controls></video>`;
...
function reviewQueueItem(item){const decision=decisionFor(item);return `<button class="review-item ${item.id===selectedReviewId?'active':''}" data-review-id="${escapeHtml(item.id)}"><span class="review-thumb">${thumbnailElement(item,'',true)}${item.assetKind==='video'?'<i class="review-video-mark">▶</i>':''}</span><span class="review-info"><span class="review-meta"><strong>${escapeHtml(item.user)}</strong><i class="platform-mini ${escapeHtml(item.platform)}">${escapeHtml(item.platform)}</i></span><b>${escapeHtml(item.type)} · ${escapeHtml(item.date)}</b><small>${escapeHtml(item.caption||'Captured campaign content')}</small></span><span class="confidence ${escapeHtml(decision)}">${escapeHtml(decision)}</span></button>`}
...
function bindReviewMedia(){
  const video=reviewFocus.querySelector('.review-viewer video');if(!video)return;
  const playButton=reviewFocus.querySelector('.review-play-button');const status=reviewFocus.querySelector('.review-media-status');
  const ready=()=>{status.classList.remove('show','error');status.textContent='';playButton.classList.remove('loading')};
  const failed=()=>{status.classList.add('show','error');status.innerHTML='<b>Video could not load.</b><button type="button">Retry</button>';status.querySelector('button').addEventListener('click',()=>{status.className='review-media-status show';status.textContent='Retrying video...';video.load()})};
  if(video.readyState>=3)ready();else{status.classList.add('show');playButton.classList.add('loading')}
  video.addEventListener('canplay',ready);video.addEventListener('error',failed);video.addEventListener('stalled',()=>{status.classList.add('show');status.textContent='Video is taking longer than expected...'});
  video.addEventListener('playing',()=>playButton.classList.add('playing'));video.addEventListener('pause',()=>{if(video.currentTime<video.duration)playButton.classList.remove('playing')});
  playButton.addEventListener('click',async()=>{try{await video.play();playButton.classList.add('playing')}catch{failed()}});
}
...
async function loadBackend(){
  try{
    ...
    if(isAdmin){document.getElementById('totalPiecesLabel').textContent='pieces found';document.getElementById('storyCountLabel').textContent='scraped';document.getElementById('postCountLabel').textContent='scraped';document.getElementById('tiktokCountLabel').textContent='scraped';document.getElementById('qualityLabel').textContent='Qualified after review';document.getElementById('pendingLabel').textContent=(summary.pending||0)+' pending review';document.getElementById('statScraped').textContent=summary.totalScraped||0;document.getElementById('statRelevant').textContent=summary.qualified||0;document.getElementById('statDiscarded').textContent=summary.discarded||0;document.getElementById('statPending').textContent=summary.pending||0}else{document.getElementById('pendingLabel').textContent='Relevant content only';document.getElementById('contentStats').innerHTML=`<span><strong>${summary.qualified||0}</strong> approved pieces</span><span><strong>${creators.length}</strong> approved creators</span>`}
