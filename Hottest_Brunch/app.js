...
function deckMediaCard(item,index){
  ...
  return `<article class="deck-media-card platform-${platform}"><div class="deck-media-frame">${asset}<div class="deck-media-top"><div><span class="deck-media-index">${String(index+1).padStart(2,'0')}</span><span class="deck-platform ${platform}"><i>${platformIcon}</i>${platformName}</span></div><b>${item.type}</b></div></div><div class="deck-media-copy"><span>${platformName} · ${item.date||'CAPTURED'}</span><p>${item.caption||'Captured campaign content'}</p><div><b>${item.views}</b><small>views</small><b>${item.likes}</b><small>likes</small></div></div></article>`
}
